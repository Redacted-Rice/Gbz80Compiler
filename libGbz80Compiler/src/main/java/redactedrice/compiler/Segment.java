...
public class Segment {
    ...
    public int getWorstCaseSize(BankAddress segmentAddress, AssignedAddresses assignedAddresses,
            AssignedAddresses tempIndexes) {
        ...
        for (Instruction item : data) {
            ...
            if (!instructAddr.offset(
                    item.getWorstCaseSize(instructAddr, assignedAddresses, tempIndexes),
    ...
    public int writeBytes(QueuedWriter writer, BankAddress segmentStartAddress,
            AssignedAddresses assignedAddresses) throws IOException {
        BankAddress instructAddress = new BankAddress(segmentStartAddress);
        for (Instruction item : data) {
            instructAddress.offset(item.writeBytes(writer, instructAddress, assignedAddresses),
