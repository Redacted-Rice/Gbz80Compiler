...
public class BlockBankLoadedAddress extends AddressRefInstruction {
    ...
    public int writeBytes(QueuedWriter writer, BankAddress unused,
        ...
        if (!address.isFullAddress()) {
            throw new IllegalAccessError("BlockBankLoaded Address tried to write address for "
    ...
    public static void write(QueuedWriter writer, BankAddress toWrite) throws IOException {
        writer.append(ByteUtils
                .shortToLittleEndianBytes(RomUtils.convertFromBankOffsetToLoadedOffset(toWrite)));
    }
}
