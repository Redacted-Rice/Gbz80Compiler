...
public class Jr extends JumpCommon {
    ...
    public static Jr create(String[] args, String rootSegment) {
        ...
        final String offsetError = "jr cannot accept a offset for internal calls, use a reference and for writing over data see nop - "
                + supportedArgs;
    ...
    public static void blankWithJrs(QueuedWriter writer, int bytesToSkip) throws IOException {
        ...
        while (leftToSkip > 0) {
            ...
            // If its larger than our max jump (plus instruct size since its relative to end of the
            // instruct)
    ...
    public static int write(QueuedWriter writer, InstructionConditions conditions, byte relAddress)
            throws IOException {
