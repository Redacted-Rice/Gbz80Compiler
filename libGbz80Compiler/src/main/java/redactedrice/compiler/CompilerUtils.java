...
public final class CompilerUtils {
    ...
    public static String extractHexValString(String arg, int maxNumChars, int offsetChars) {
        ...
        if (valIdx < HEX_VAL_STANDARD_PREFIX.length()) {
            ...
            if (valIdx <= HEX_VAL_GBZ80_PREFIX.length()) {
                throw new IllegalArgumentException(
                        "Failed to find either " + HEX_VAL_STANDARD_PREFIX + " or "
                                + HEX_VAL_GBZ80_PREFIX + " hex value marker: " + arg);
            }
        }
        ...
        // Get the base string, split on space and return the first in case we overflowed into
        // another arg
        ...
        throw new IllegalArgumentException(
                "Failed parse string - extracted string is not hex: " + hexString);
    }
    ...
    public static short parseMemoryAddressArg(String arg) {
        ...
        throw new IllegalArgumentException(
                "Passed arg is not surrounded in brackets so it is not a valid memory address: "
                        + arg);
    }
